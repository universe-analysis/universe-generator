...
#### Super-Speculation:
If this was a model of our universe, we could expect it to imply that the observable universe is the entire universe. This is now measured rather than speculative (2026-07-16): the model's causal structure is set by the wiggle budget, not a null cone, and the comoving reach has closed forms — ≈1.26 near the Bang (already above the antipode distance 1), √2 at the turnaround, 2 at the Crunch. Antipodal points are connectable arbitrarily close to the Bang, so there is **no horizon at any epoch**. (An earlier version of this claim — that closing the maximum distance takes the entire first half of time — was disproven by that analysis.) It would also imply that expansion is not accelerating but instead slowing. These ideas should be able to be examined and potentially tested further.
