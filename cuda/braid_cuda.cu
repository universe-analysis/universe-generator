...
//     sum |a_j*b_j| = 1 on the wiggle group (sin1 is a free comoving offset).
//   * Collisions are tested in comoving coordinates X = x/sin(z): Chebyshev
//     square of half-width CELL = 2/T, per timestep, over the periodic
//     domain [-1, 1)^2 (a torus; the only geometry since 2026-07-09, when
//     the legacy hard wall was removed -- --torus is an accepted no-op).
//
...
struct Path {
    ...
    double fx[kMaxWiggle], fy[kMaxWiggle];  // wiggle phases (0 on odd frequencies)
...
__device__ bool collides_dev(const Path& p,
    ...
    for (int oi = 0; oi < T; oi++) {
        ...
        // Wrap onto the fundamental domain [-1, 1) and index the grid; the
        // 3x3 neighbour scan wraps modulo the grid.
        X = torus_wrap(X);
        Y = torus_wrap(Y);
...
int main(int argc, char** argv) {
    ...
    auto gix = [&](double v) -> int { return (int)floor((v + 1.0) / cell); };
    ...
    // Under the full-spectrum cap (KMAX_WIGGLE > 16, Path ~7 KB) the buffer
    // shrinks to the initial batch size -- still an upper bound on any
    // round's survivor count until the batch grows, which only happens once
    // the acceptance rate is far below cap/batch.
    int survCap = kMaxWiggle > 16 ? 1 << 16 : 1 << 18;
    Path* dSurv;
    int* dSurvCount;
    ...
    while (attempts < (long long)budget && !(maxN && N >= maxN)) {
        ...
        test_kernel<<<blocks, threads>>>(seed, round, (int)batch, modmax, nw, false, dPtsGid, T,
                                         dz, dsinz, dinvz, cell, gw, dCellStart, dCellLen,
    ...
    if (subpaths) {
        ...
        while (attemptsSub < (long long)subBudgetEff) {
            ...
            CK(cudaMemset(dSurvCount, 0, 4));
            int threads = 256, blocks = (int)((batch + threads - 1) / threads);
            test_kernel<<<blocks, threads>>>(seed, round, (int)batch, modmax, nwSub, true,
                                             dPtsGid, T, dz, dsinz, dinvz, cell, gw, dCellStart,
                                             dCellLen, dPtsX, dPtsY, dorder, dSurv, dSurvCount,
                                             survCap);
            CK(cudaDeviceSynchronize());
            int sc;
            CK(cudaMemcpy(&sc, dSurvCount, 4, cudaMemcpyDeviceToHost));
